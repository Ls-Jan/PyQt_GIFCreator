...
class SavingUI(QWidget):
	...
	def __init__(self):
		...
		self.__worker.finished.connect(self.__Action_Worker_Finish)
	...
	def __Action_Worker_Finish(self):
	...
	def __Action_UpdateHint(self):
		...
		memSize=self.__worker.Get_MemorySize()
		writtenSize=self.__worker.Get_WrittenSize()

		percent=writtenSize/gifSize*100 if gifSize!=0 else 0
		self.__hintMemory.setText(self.config['hintMemory'].format(size=self.__CalcSize(memSize if gifSize==0 else 0)))
		self.__hintSize.setText(self.config['hintSize'].format(size=self.__CalcSize(gifSize)))
		self.__hintStatus.setText('' if status==0 else self.config['hintCreate'] if status==1 else self.config['hintSave'])
